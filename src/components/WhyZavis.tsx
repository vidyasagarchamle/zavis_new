...
export default function WhyZavis() {
  ...
  return (
    <section id="whyZavis" className="py-10 sm:py-12 md:py-16 bg-transparent relative" ref={containerRef}>
      ...
      <div className="container mx-auto px-4">
        ...
        {/* Comparison table */}
        <motion.div 
          className="max-w-4xl mx-auto mb-8 sm:mb-12 md:mb-16 overflow-hidden rounded-xl border border-border"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          <div className="bg-background-secondary p-4 sm:p-6">
            <div className="grid grid-cols-3 gap-2 sm:gap-4">
              ...
              <div className="col-span-1 text-center">
                <div className="inline-block bg-background/50 rounded-lg px-2 sm:px-4 py-1 sm:py-2 text-[color:var(--foreground-secondary)] text-xs sm:text-sm">
                  Others
                </div>
              </div>
            </div>
          ...
          <div className="divide-y divide-border">
            {[
              ...
              <div key={index} className="grid grid-cols-3 gap-2 sm:gap-4 p-3 sm:p-4">
                <div className="col-span-1 flex items-center">
                  <span className="font-medium text-xs sm:text-sm">{item.feature}</span>
                </div>
                <div className="col-span-1 flex items-center justify-center">
                  {typeof item.zavis === "boolean" ? (
                    item.zavis ? (
                      ...
                      <svg className="w-5 h-5 sm:w-6 sm:h-6 text-[color:var(--foreground-secondary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    )
                  ) : (
                    <span className="text-accent font-medium text-xs sm:text-sm">{item.zavis}</span>
                  )}
                </div>
                <div className="col-span-1 flex items-center justify-center">
                  {typeof item.others === "boolean" ? (
                    ...
                    <span className="text-[color:var(--foreground-secondary)] text-xs sm:text-sm">{item.others}</span>
                  )}
                </div>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 max-w-4xl mx-auto">
          {differentiators.map((item, index) => (
            <motion.div
              ...
              className="bg-background border border-border rounded-xl p-4 sm:p-6 hover:shadow-glow transition-all duration-300"
            >
              <div className="flex items-start">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-accent/10 flex items-center justify-center text-accent mr-3 sm:mr-4">
                  {item.icon}
                </div>
                <div>
                  <h3 className="text-base sm:text-lg md:text-xl font-bold mb-1 sm:mb-2">{item.title}</h3>
                  <p className="text-xs sm:text-sm text-[color:var(--foreground-secondary)]">{item.description}</p>
                </div>
              </div>
            </motion.div>
          ))}
